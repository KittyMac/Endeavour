...
class DocumentsDelegate: Actor, Documentable {
    ...
    private func _beSave(documentInfo: DocumentInfo) -> Error? {
        do {
    ...
    private func _beRevert(documentInfo: DocumentInfo,
...
extension DocumentsDelegate {

    @discardableResult
    public func beSave(documentInfo: DocumentInfo,
                       _ sender: Actor,
                       _ callback: @escaping ((Error?) -> Void)) -> Self {
        unsafeSend {
            let result = self._beSave(documentInfo: documentInfo)
            sender.unsafeSend { callback(result) }
        }
        return self
    }
    @discardableResult
    public func beRevert(documentInfo: DocumentInfo,
                         _ sender: Actor,
                         _ callback: @escaping ((DocumentContent?, Error?) -> Void)) -> Self {
        unsafeSend {
            self._beRevert(documentInfo: documentInfo) { arg0, arg1 in
                sender.unsafeSend {
                    callback(arg0, arg1)
                }
            }
